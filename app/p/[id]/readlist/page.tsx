...
export default function ProfileReadListPage() {
    ...
    useEffect(() => {
        async function load() {
            setLoading(true);
            try {
                const res = await fetch(`/api/p/${encodeURIComponent(profileId)}/readlist`, {
                    cache: "no-store",
                });

                if (!res.ok) {
                    console.error("Kullanıcı okuma listesi yüklenemedi");
                    setLoading(false);
                    return;
                }

                const data = await res.json();
                // The API returns 'readList', so we use that.
                const bookIds: string[] = data.readList || [];
        ...
        <div className="min-h-screen text-white w-full">
            <div className="max-w-6xl mx-auto w-full">
                ...
                <div className="px-4 py-8 space-y-8 w-full">
                    {loading ? (
                        <p className="text-gray-500">Okuma listesi yükleniyor…</p>
                    ) : sortedBooks.length === 0 ? (
                        <p className="text-gray-500">Okuma listesinde henüz kitap yok.</p>
                    ) : (
                        <>
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                <div className="flex flex-col gap-1">
                                    <h2 className="text-xl font-semibold">Okuma Listesi</h2>
